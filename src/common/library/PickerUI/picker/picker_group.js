...
class PickerGroup extends Component {
    ...
    render() {
        ...
        const cls = `weui-picker__group ${className}`
        const styles = {
            WebkitTransform: `translate(0, ${this.state.translate}px)`,
            transform: `translate(0, ${this.state.translate}px)`,
            transition: this.state.animating ? 'transform .3s' : 'none',
            WebkitTransition: this.state.animating ? 'transform .3s' : 'none'
        };
        return (
            <div
                className={`${cls} date_picker_user_select`}
            ...
            >
                ...
                >
                    {items.map((item, j) => {
                        ...
                        const itemCls = `weui-picker__item ${ item.disabled && 'weui-picker__item_disabled'} ${selected === j && 'weui-picker__item_selected'}`
                        return (
